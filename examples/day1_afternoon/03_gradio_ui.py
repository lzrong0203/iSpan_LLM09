...
def create_chat_interface():
    ...
    def chat_with_ai(message, history):
        ...
        try:
            # 建立訊息列表
            messages = [{"role": "system", "content": "你是一個友善的AI助手"}]
            ...
            with gr.Row():
                with gr.Column():
                    chat_input = gr.Textbox(
                        label="輸入訊息", placeholder="問我任何問題...", lines=3
                    )
                    temp_slider = gr.Slider(
                        minimum=0, maximum=1, value=0.7, label="創意度 (Temperature)"
                    )
                ...
                with gr.Column():
                    chat_output = gr.Textbox(label="AI回應", lines=8)
            ...
            with gr.Row():
                with gr.Column():
                    trans_input = gr.Textbox(
                        label="原文", placeholder="輸入要翻譯的文字...", lines=5
                    )
                ...
                with gr.Column():
                    trans_output = gr.Textbox(label="翻譯結果", lines=5)
            ...
            with gr.Row():
                with gr.Column():
                    summary_input = gr.Textbox(
                        label="原文", placeholder="貼上要摘要的長文...", lines=10
                    )
                ...
                with gr.Column():
                    summary_output = gr.Textbox(label="摘要結果", lines=5)
        ...
        with gr.Row():
            with gr.Column(scale=2):
                input_text = gr.Textbox(
                    label="輸入文字", placeholder="輸入你要處理的文字...", lines=3
                )
                ...
                process_btn = gr.Button("處理文字", variant="primary", size="lg")

            with gr.Column(scale=2):
                output_text = gr.Textbox(label="處理結果", lines=3, interactive=False)
...
demo.launch(
    server_name="127.0.0.1", server_port=7860, share=False  # 設為True可取得公開連結
)
